...
class Header extends Component {
  ...
  componentDidMount () {
    this.fetchText('header', 'russian', 'Ru')
    this.fetchText('header', 'english', 'Eng')
  }
  ...
  fetchText (page, language, suff) {
    const { words } = this.state
    firebase.database().ref('siteInfo/' + `${page}/` + `${language}`)
    .once('value')
    .then(snapshot => {
      ...
      if (object !== null) {
        if (suff === 'Ru') {
          let wordsRu = {}
          words.forEach(item => {
            wordsRu[`${item}${suff}`] = object[`${item}${suff}`]
          })
          this.setState({ wordsRu, siteInfoLoaded: true })
        } else if (suff === 'Eng') {
          let wordsEng = {}
          words.forEach(item => {
            wordsEng[`${item}${suff}`] = object[`${item}${suff}`]
          })
